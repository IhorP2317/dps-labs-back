...
namespace dbs_labs_back.Endpoints ;
    ...
    public class DsaEndpoints(DSAUtil dsaUtil):ICarterModule
    {
        ...
        private  IResult GetSignatureFromString([FromQuery] string input) => Results.Ok(dsaUtil.ProcessSignature(Encoding.ASCII.GetBytes(input)));
        ...
        private async Task<IResult> GetSignatureFromFile([FromQuery] string fileName)
        {
            ...
            return Results.Ok(dsaUtil.ProcessSignature(await File.ReadAllBytesAsync(absoluteFilePath)));
        }
        private async Task<IResult> VerifySignature([FromQuery] string dataFileName, [FromQuery] string signFileName)
        {
            ...
            return Results.Ok(dsaUtil.VerifySignature(await File.ReadAllBytesAsync(absoluteDataFilePath),signature));
        }
    }